...
LL=floor(length(w)/2)*2;
i=1:2:length(w)-1; j=2:2:LL;
GPSTimei=(GPSTime(i)+GPSTime(j))/2;
wi=(w(i)+w(j))/2;
conc2=interp1q(cmig(:,2),conccm,GPSTimei);
conc22=speeditup(conc2);
plot(GPSTime, conc,'.-',cmig(:,2),conccm,'.-', GPSTimei,conc22,'.-',GPSTimei,conc2,'.-')
pause
s7=(['save ' filepath3 'Gci' filename3]);   % Gc contains c-migit data, Gci contains 10 Hz data conc speeded
heading=interp1q(cmig(:,2),cmig(:,11),GPSTime);
pitch=interp1q(cmig(:,2),cmig(:,9),GPSTime);
roll=interp1q(cmig(:,2),cmig(:,10),GPSTime);
conc2=conc22;
clear s2 filename3 filepath3 conccm part2 part j s5 s6 s4 s3 cpath conc55 tconc55 conc22
