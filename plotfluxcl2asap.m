...
s=['load C:\ocean2006\fluxes\xwiflux200' num2str(y) mm ddd d(5:6) 'T200.mat'];
   eval(s)
   clear s
   ...
   clear s k1000
        ...
        s=['load C:\ocean2006\hrad\aerorad\chl200' num2str(y) '_' mm '_' ddd 'cloudDA.dat'];
   eval(s)
   s=['datclc=chl200' num2str(y) '_' mm '_' ddd 'cloudDA;'];
   eval(s)
   s=['clear chl200' num2str(y) '_' mm '_' ddd 'cloudDA;'];
   eval(s)
   k1000=find(datclc(:,5)>1000);
   datclc(k1000,:)=[];
   length(k1000)
  ...
  y=num2str(y);
   ...
   h=find(Malt<50 & MSST<292); h2=find(Malt<50 & Fp3b+Fph+Fpw>-2 & Fp3b+Fph+Fpw<2 & MSST<292);
   subplot(2,2,1)
   ...
   contourPlotJK('SST', 'Longitude', 'Latitude', SST(h3)-273.16, longitude(h3)*180/pi, latitude(h3)*180/pi, 0.045, 0.045, 10, 0, 0, [floor(min(SST(h3))-273.15), min(ceil(max(SST(h3))-273.15), 18), 0.5])
  ...
  grid
   ...
   hold off
   subplot(2,2,3)
   contourPlotJK('aerosol number' , 'Longitude', 'Latitude', conc(h3), longitude(h3)*180/pi, latitude(h3)*180/pi, 0.045, 0.045)
   orient landscape
   hold on 
    plot(coastline(:,1), coastline(:,2),'k')
   hold off
