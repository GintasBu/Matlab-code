...
day=[111802 102801 100901 92503 90801 82904 20402  80801 80902 81001 81101 81501 90409 90501 102801 31401 12701 21701 120501];
load coastline.txt
...
if length(d)==5, d=['0' d]; end
   ...
   if day(flight)==20402 || day(flight)==12701 || day(flight)==31401, y=4; end
   if day(flight)==21701, y=4; end
  
s=['load C:\ocean2006\fluxes\AOSN\xwiflux200' num2str(y) mm ddd d(5:6) 'T60.mat']
s2=['load C:\ocean2006\hrad\aerorad\chl200' num2str(y) '_' mm '_' ddd '.dat']
 eval(s)
   ...
   s3=['datcl1=chl200' num2str(y) '_' mm '_' ddd ';']
   eval(s3)
   s4=['clear chl200' num2str(y) '_' mm '_' ddd ';']
   eval(s4)
  ...
  y=num2str(y);
   s=['save C:\ocean2006\fluxes\AOSN\xwiclflux200' y mm ddd d(5:6) 'T60b.mat'];
   eval(s)
   
   clear k d mm dd y  % datclc datcl1  ddd utc
   %end
   
   h=find(Malt<50); h2=find(Malt<50 & Fp3b+Fph+Fpw>-1 & Fp3b+Fph+Fpw<1);
   subplot(2,2,1)
   ...
   grid
    subplot(2,2,2)
   %figure
   contourPlotJK(day(flight), 'Longitude', 'Latitude', datclc(:,5), datclc(:,3), datclc(:,2), 0.045, 0.045, 10, 1, 0)
   legend('chlorophyl2')
    orient landscape
    hold on 
    plot(coastline(:,1), coastline(:,2),'k')
   ...
   %figure
    subplot(2,2,3)
       if day(flight)==92503 ,
        contourPlotJK('part flux, filtered' , 'Longitude', 'Latitude', (Fp3b(h2)+Fph(h2)+Fpw(h2)), Mlong(h2)*180/pi, Mlat(h2)*180/pi, 0.045, 0.045, 10, 1, 1,[-1,1,0.25])
       else if day(flight)==90409,
                contourPlotJK('part flux, filtered' , 'Longitude', 'Latitude', (Fp3b(h2)+Fph(h2)+Fpw(h2)), Mlong(h2)*180/pi, Mlat(h2)*180/pi, 0.045, 0.045,10,0,0,[-2,2,0.5])
           else
          contourPlotJK('part flux, filtered' , 'Longitude', 'Latitude', (Fp3b(h2)+Fph(h2)+Fpw(h2)), Mlong(h2)*180/pi, Mlat(h2)*180/pi, 0.035, 0.035, 10, 1, 0,[-1,1,0.25])
           end
       end
   ...
   contourPlotJK('SST', 'Longitude', 'Latitude', SST(h3)-273.16, longitude(h3)*180/pi, latitude(h3)*180/pi, 0.025, 0.025, 10, 1, 0, [floor(min(SST(h3))-273.15), min(ceil(max(SST(h3))-273.15), 18), 0.5])
   ...
   subplot(2,2,1)
  contourPlotJK(day(flight), 'Longitude', 'Latitude', datcl1(:,5), datcl1(:,3), datcl1(:,2), 0.025, 0.025, 10, 1, 0)
   legend('chlorophyl')
    orient landscape
   hold on 
   grid
   ...
   subplot(2,2,2)
   if day(flight)==90409 ,
        contourPlotJK('full part flux' , 'Longitude', 'Latitude', (Fp3b(h)+Fph(h)+Fpw(h)), Mlong(h)*180/pi, Mlat(h)*180/pi, 0.045, 0.045)
       else
        contourPlotJK('full part flux' , 'Longitude', 'Latitude', (Fp3b(h)+Fph(h)+Fpw(h)), Mlong(h)*180/pi, Mlat(h)*180/pi, 0.035, 0.035, 10, 1, 0,[-10,10,1])
   end
  grid
   orient landscape
   hold on 
   ...
   hold off
   subplot(2,2,3)
   contourPlotJK('aerosol number' , 'Longitude', 'Latitude', conc(h3)/2.35, longitude(h3)*180/pi, latitude(h3)*180/pi, 0.025, 0.025, 10, 1, 0)
   orient landscape
   hold on 
    plot(coastline(:,1), coastline(:,2),'k')
   hold off
   ...
   grid
  subplot(2,2,4)
   plot3(longitude(h3)*180/pi,latitude(h3)*180/pi, conc(h3)/2.35,'.')
grid
...
%subplot(3,1,1)
 ...
 %   plot(coastline(:,1), coastline(:,2),'k')
  % hold off
  % grid
   h33=find(Malt<60 & MSST<293);
   figure
   contourPlotJK(day(flight) , 'Longitude', 'Latitude', H(h33), Mlong(h33)*180/pi, Mlat(h33)*180/pi, 0.025, 0.025, 10, 1, 0)
   legend('Heat flux, W m^-^2')
  grid
   ppp=['print -djpeg c:/ocean2003/figures//heatflux20040' num2str(day(flight))];
  % eval(ppp)
