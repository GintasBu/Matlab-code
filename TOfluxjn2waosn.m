...
Ns=sum(isnan(N));
jn2=find(isnan(N)>0);
N=N*10/16.67/2.35;   % sampled at 10 Hz with 2.35 lpm flow rate. brings to #/cc
Ni=speeditup(N);
...
clear t u v w T N Ni longitude latitude altitude heading pitch roll jn2 Pstatic r_Irga
...
N(jn)=mean(N(jnn));
jni=find(Ni<0); jnni=find(Ni>0);
Ni(jni)=mean(Ni(jnni));
clear jnn jn jni jnni
jn=find(N>0); jni=find(Ni>0);
    %break
    %end
...
ri=xcorrTO(w, Ndi,plagi(1),plagi(2));
ji=find(abs(ri(:,2)) == max(abs(ri(:,2))));
lagpi(ii)=ri(ji,1);

%display('laged OK')
Fp(ii)=covar2(w,Nd,lagp);
Fp2(ii)=covar2(w,Nd,33);
Fp3(ii)=covar2(w(jni),Ndi(jni),lagpi);
Fp3b(ii)=covar2(w(jni),Ndi(jni),33);
